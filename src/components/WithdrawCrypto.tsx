...
const WithdrawCrypto = ({
  ...
  const handleContinue = () => {
    ...
    if (parsedAmount > balance) { toast.error("Insufficient balance"); return; }
    setStep("review");
  };
  ...
  const handleDone = () => { setStep("input"); setAddress(""); setAmount(""); onClose?.(); };
  ...
  if (step === "success") {
    const receiptText = `Withdrew ${parsedAmount.toFixed(2)} USDC to ${truncateAddr(address.trim())} on ${selectedChain.name}`;
    ...
    return (
      <div className="flex flex-col items-center py-8">
        ...
        <Button variant="outline" onClick={copyReceipt} className="w-full rounded-2xl h-10 text-xs font-semibold border-border/60 mb-3">
          <Copy className="h-3.5 w-3.5 mr-1.5" /> Copy Receipt
        </Button>
        <Button onClick={handleDone} className="w-full rounded-2xl h-12 text-sm font-semibold">Done</Button>
      </div>
    );
  }
  ...
  if (step === "review") {
    return (
      <div className="flex flex-col">
        <h2 className="text-lg font-bold text-foreground text-center mb-6">Confirm Withdrawal</h2>
        <div className="w-full rounded-2xl bg-card border border-border/60 overflow-hidden mb-6">
          ...
          <div className="flex items-center justify-between px-4 py-3.5 border-b border-border/40">
            ...
            <span className="text-xs font-mono text-muted-foreground">{truncateAddr(address.trim())}</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button onClick={handleSend} disabled={isLoading} className="rounded-2xl h-12 text-sm font-semibold">
            {sending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null} Withdraw
          </Button>
        </div>
      </div>
    );
