 ...
 *
 * Uses a popup (not an iframe) — Noah's hosted checkout blocks its
 * interactive Terms/consent step when framed, so a popup is the only
 * embeddable option available for this flow.
 */
const KycBanner = () => {
  ...
  const openHosted = (rawUrl: string) => {
    ...
    if (!url) { toast.error("Could not open verification securely. Please try again."); return; }
